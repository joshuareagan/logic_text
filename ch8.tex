...
For any sequence of derivation lines that is a derivation, the sentence $\CAPPHI$ on the last line is entailed by the set $\Delta$ of sentences that are on unboxed lines and are sanctioned by \Rule{Assume}.\footnote{See definition \pmvref{Recursive definition of Derivation}.} 
\end{THEOREM}
\noindent{}Since the definition of a derivation is a recursive definition\footnote{See def. \pmvref{Recursive definition of Derivation}.}, the most natural way to prove theorem \ref{Main GSL Soundness Lemma} is through a recursive proof. 
...
\item[Base Step:] 
The base case is a single-line derivation $\Derivation{D}$ sanctioned by the rule \Rule{Assume}. 
Say the sentence on that line is $\CAPPHI$.
We have to show that the sentence on the last line is entailed by all the sentences, on unboxed lines, that are sanctioned by \Rule{Assume}. 
But in this case the sentence on the last line is $\CAPPHI$, and the set of unboxed sentences sanctioned by \Rule{Assume} only contains $\CAPPHI$. 
...
\item[\Rule{Assume}:] 
Say we add another line to $\Derivation{D}$ with sentence $\CAPPHI$ sanctioned by \Rule{Assume}. 
...
Like \Rule{Assume}, the assumptions change in \Rule{$\HORSESHOE$-Intro}. 
...
The rule which sanctions these steps will be \Rule{Assume}. 
