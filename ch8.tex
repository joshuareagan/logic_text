...
\begin{commentary}
	...
	Then we reorder the conjuncts of each disjunct using \Rule{$\wedge$-Shuffle} to isolate the conjuncts with a matching variable, and use Q shuffle to move the quantifier to govern just those.
	\commentaryspace
	...
	Then we reorder the disjuncts of each canjunct using \Rule{$\vee$-Shuffle} to isolate the disjuncts with a matching variable, and use Q shuffle to move the quantifier to govern just those.
...
We push in each $\#_i$, transforming the subformula $\CAPPHI'$ into a new formula $\CAPPHI_i$ with $\#_i$ moved all the way in.
Let $\CAPPHI_{n+1}$ be $\CAPPHI'$, i.e. the original DNF body of $\CAPPHI$.
