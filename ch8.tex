...
Since \GQL{} has many-place predicates, if one quantifier occurs within the scope of another then it cannot be moved in front of the other quantifier. 
But since all the predicates of \GQL{}1 are 1-place there is a procedure to switch the order of these quantifiers.
...
To prove this we introduce a new normal form and some additional exchange rules.

\subsection{Conjunctive Normal Form}\label{Conjunctive Normal Form}

Back in section \ref{Disjunctive Normal Form} you were introduced to sentences in disjunctive normal form (DNF).
Now we consider a closely related form.

\begin{majorILnc}{\LnpDC{CNF Definition}}
A \GQL{} formula without quantifiers is in \df{conjunctive normal form} (\CAPS{cnf})\index{CNF|see{conjunctive normal form}} \Iff
\begin{cenumerate}
\item it contains no conditional ($\HORSESHOE$) or biconditional ($\TRIPLEBAR$),
\item negations ($\NEGATION$) only govern atomic formulas, and
\item no disjunction ($\VEE$) contains a conjunction ($\WEDGE$) as a subformula.
\end{cenumerate}
\end{majorILnc}
\noindent{}An example of a sentence in \CAPS{cnf} is $\conjunction{\pardisjunction{\Ql}{\negation{\Rl}}}{\pardisjunction{\negation{\Pl}}{\Rl}}$.

The definition of CNF differs from DNF only in the third part of the definition.
To get a DNF sentence, use the distribution exchange rule to get a disjunction of conjunctions.
To get CNF, use the same rule to get a conjunction of disjunctions.
A formula can be converted from DNF to CNF or \emph{vice versa} by (possibly repeated) application of the distribution rule.

A \GQL{} sentence is in \idf{prenex conjunctive normal form}\index{conjunctive normal form!prenex} (\CAPS{pcnf}) \Iff is it's in prenex normal form (i.e. all the quantifiers are in the initial position) and the remainder is in CNF.

\begin{THEOREM}{\LnpTC{Prenex Conjunctive Normal Form Theorem} The Prenex Conjunctive Normal Form Theorem:}
Every sentence of \GQL{} is equivalent to an \GQL{} sentence in \CAPS{pcnf}.
\end{THEOREM}
\begin{PROOF}
The proof relies on four lemmas (left to the reader) corresponding to the last four steps of the procedure below.
We provide a five stage process showing how to turn any given sentence into one that's in \CAPS{pcnf}.
Let $\CAPPHI$ be a sentence of \GQL{}.
\begin{description}
\item[Step A:] There is a prenex normal form equivalent of $\CAPPHI$, $\CAPPHI^*$. Let $\CAPPHI'$ be the subformula of $\CAPPHI^*$ containing everything except the quantifiers in the initial position.
\item[Step B:] If a subformula of $\CAPPHI'$ has a conditional or biconditional as its main connective, i.e., is of the form $\parhorseshoe{\CAPPSI}{\CAPTHETA}$ or $\partriplebar{\CAPTHETA}{\CAPPSI}$, replace the subformula by $\pardisjunction{\negation{\CAPPSI}}{\CAPTHETA}$ or $\disjunction{\parconjunction{\CAPPSI}{\CAPTHETA}}{\parconjunction{\negation{\CAPPSI}}{\negation{\CAPTHETA}}}$ respectively. 
Repeat as necessary to obtain a formula $\CAPPHI''$ without conditionals or biconditionals.
\item[Step C:] \hfill{}
\begin{cenumerate}
\item Replace any subformula of the form $\negation{\negation{\CAPPSI}}$ in $\CAPPHI''$ with $\CAPPSI$.
...
\end{cenumerate} 
Repeat as necessary to obtain $\CAPPHI'''$ in which negations govern nothing but atomic formulas.
\item[Step D:] The only thing that could prevent $\CAPPHI'''$ from being in \CAPS{cnf} is that some disjunctions govern some conjunctions, i.e., there is a subformula $\disjunction{\CAPTHETA}{\parconjunction{\CAPPSI_1}{\conjunction{\CAPPSI_2}{\conjunction{\ldots}{\CAPPSI_{\integer{n}}}}}}$, or the reverse $\disjunction{\parconjunction{\CAPPSI_1}{\conjunction{\CAPPSI_2}{\conjunction{\ldots}{\CAPPSI_{\integer{n}}}}}}{\CAPTHETA}$. 
Those subformulas can be replaced by the equivalent $\conjunction{\pardisjunction{\CAPPSI_1}{\CAPTHETA}}{\conjunction{\pardisjunction{\CAPPSI_2}{\CAPTHETA}}{\conjunction{\ldots}{\pardisjunction{\CAPPSI_{\integer{n}}}{\CAPTHETA}}}}$. 
Repeat as necessary to obtain a formula $\CAPPHI''''$.
\item[Step E:] Replace $\CAPPHI'$ in $\CAPPHI^*$ with $\CAPPHI''''$ to get $\CAPPHI^{**}$. $\CAPPHI^{**}$ is equivalent to $\CAPPHI$ and in prenex conjunctive normal form.
\end{description}
\end{PROOF}

\subsection{Additonal Quantifier Exchange Rules}\label{Additonal Quantifier Exchange Rules}

Our aim is to move the quantifiers of a \GQL{}1 sentence so that all are independent.
To simplify this task we introduce new quantifier exchange rules, specifically selected for use with DNF and CNF sentences.
...
\endhead
\bottomrule
\caption{Additonal Quantifier Exchange Rules for \GQD{}}\\[-.15in]
\multicolumn{3}{c}{\emph{Continued next Page}}\\
\endfoot
\bottomrule
\caption{Additonal Quantifier Exchange Rules for \GQD{}}\\
\endlastfoot
\label{AdditionalQuantifierExRules}
...
\begin{commentary}
	...
	\commentaryspace
	First, we start with a \GQL{}1 sentence and get its PCNF equivalent. 
	...
	\commentaryspace
	To push existential quantifiers in, the distribution exchange rule is first used to make the body of the sentence a disjunction.
	After the quantifier is moved in, the distribution exchange rule is applied again to make the body of the sentence a conjunction again.
	The universal quantifier can be pushed in without using distribution.
\end{commentary}
Let there be a sentence of \GQL{}1.
Then by theorem \ref{Prenex Conjunctive Normal Form Theorem} there is an equivalent sentence, $\CAPPHI$, that is in prenex conjunctive normal form.
So $\CAPPHI$ is of the form $\#_1\#_2\ldots\#_n\CAPPHI'$, where for all $i\in\{1,\ldots,n\}$ each $\#_i$ is a quantifier, and $\CAPPHI'$ is in CNF.
Then $\CAPPHI'$ is of the form $\parconjunction{\conjunction{\CAPPSI_1}{\CAPPSI_2}}{\conjunction{\ldots}{\CAPPSI_k}}$, where for each $j\in\{1,\ldots,k\}$, $\CAPPSI_j$ is a disjunction of ions.

To make the quantifiers all independent, the following steps are applied to push in each quantifier, starting with $\#_n$ and working back to $\#_1$.
Let $\#_i$ be the right-most quantifier that hasn't yet been moved in $\#_1\#_2\ldots\#_n$.
We push in each $\#_i$, transforming the subformula $\CAPPHI'$ into a new formula $\CAPPHI_i$ when that quantifier is moved in.
Let $\CAPPHI_{n+1}$ be $\CAPPHI'$, i.e. the original CNF body of $\CAPPHI$.
There are two cases, handled separately:

\vspace*{0.15in}
\noindent{}\textbf{Case 1}: $\#_i$ is a universal quantifier.

\begin{quote}
\begin{description}
\item[Step A:] By $\universal$-shuffle, $\#_i$ is moved to each conjunct in $\CAPPHI_{i+1}$ containing the variable bound by $\#_i$, resulting in: $\parconjunction{\conjunction{\#_i\CAPPSI_1}{\#_i\CAPPSI_2}}{\conjunction{\ldots}{\#_i\CAPPSI_k}}$. For any $\CAPPSI_j$ not containing that variable, the quantifier may be removed, by Q-deletion.
\item[Step B:] Each conjunct with a quantifier $\#_i\CAPPSI_j$ is of the form \\ $\#_i\pardisjunction{\disjunction{\CAPTHETA_1}{\CAPTHETA_2}}{\disjunction{\ldots}{\CAPTHETA_m}}$ where each $\CAPTHETA$ is an ion (or a quantified subsentence governing a conjunction or disjunction, as will be made clear shortly). An equivalent disjunction is obtained by reordering so that the disjuncts with a variable matching that of the quantifier are leftmost. By the Q shuffle exchange rule, the $\#_i$ is moved in to govern just those disjuncts, resulting in $\#_i\CAPPSI_j$ becoming: $\disjunction{\#_i\pardisjunction{\CAPTHETA_1}{\CAPTHETA_2}}{\disjunction{\ldots}{\CAPTHETA_m}}$. With this transformation $\#_i$ binds all variables within its scope, and no other variables are in its scope.
\end{description}
\noindent{}The result is a formula $\CAPPHI_{i}$ where $\#_i$ binds all and only its variables.
\end{quote}

\noindent{}\textbf{Case 2}: $\#_i$ is a existential quantifier.

\begin{quote}
\begin{description}
\item[Step A:] Apply the distribution rule to $\CAPPHI_{i+1}$ until no disjunction is governed by a conjunction (with the exception of any disjunction within the scope of an already moved quantifier). The resulting formula, $\CAPPHI_{i+1}^*$, is a disjunction.
\item[Step B:] By $\existential$-shuffle, $\#_i$ is moved to each disjunct in $\CAPPHI_{i+1}^*$ containing the variable bound by $\#_i$, resulting in: $\pardisjunction{\disjunction{\#_i\CAPPSI_1}{\#_i\CAPPSI_2}}{\disjunction{\ldots}{\#_i\CAPPSI_k}}$. For any $\CAPPSI_j$ not containing that variable, the quantifier may be removed, by Q-deletion.
\item[Step C:] Each disjunct with a quantifier $\#_i\CAPPSI_j$ is of the form \\ $\#_i\parconjunction{\conjunction{\CAPTHETA_1}{\CAPTHETA_2}}{\conjunction{\ldots}{\CAPTHETA_m}}$ where each $\CAPTHETA$ is an ion (or a quantifier subsentence governing a disjunction or conjunction). An equivalent conjunction is obtained by reordering so that the conjuncts with a variable matching that of the quantifier are leftmost. By the Q shuffle exchange rule, the $\#_i$ can be moved in to govern just those conjuncts, resulting in $\#_i\CAPPSI_j$ becoming: $\conjunction{\#_i\parconjunction{\CAPTHETA_1}{\CAPTHETA_2}}{\disjunction{\ldots}{\CAPTHETA_m}}$. With this transformation $\#_i$ binds all variables within its scope, and no other variables are in its scope.
\item[Step D:] Apply the distribution rule to the resulting formula until no conjunction is governed by a disjunction (with the exception of any conjunction within the scope of an already moved quantifier).
\end{description}
\noindent{}The result is a formula $\CAPPHI_{i}$ where $\#_i$ binds all and only its variables.
\end{quote}

\noindent{}When all quantifiers are pushed in, the result is a \GQL{}1 sentence whose quantifiers are all independent.
\end{PROOF}
