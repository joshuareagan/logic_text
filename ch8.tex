...
To prove this we introduce a new normal form.
...
\end{cenumerate}
\end{majorILnc}
\noindent{}This differs from DNF only in the third part of the definition.
An example of a sentence in \CAPS{cnf} is $\conjunction{\pardisjunction{\Ql}{\negation{\Rl}}}{\pardisjunction{\negation{\Pl}}{\Rl}}$.
...
\begin{PROOF}
\begin{commentary}
	The basic strategy to prove this theorem is straightforward.
	First, we start with a \GQL{}1 and get its PCNF equivalent. 
	All the quantifiers are in the initial position of the latter sentence.
	Then we push each quantifier inward, using the prenex exchange rules, to shrink its scope.
	We show that after the quantifiers are pushed in they are all independent.
\end{commentary}
Let $\CAPPHI$ be a sentence of \GQL{}1.
...
Let $\#_i$ be the right-most quantifier remaining to the left of $\CAPPHI'$.
\begin{description}
\item[Step A:] By the Q shuffle prenex exchange rule, $\#_i$ can be moved to each conjunct in $\CAPPHI'$ containing the variable bound by $\#_i$, resulting in: $\parconjunction{\conjunction{\#_i\CAPPSI_1}{\#_i\CAPPSI_2}}{\conjunction{\ldots}{\#_i\CAPPSI_k}}$. For any $\CAPPSI_j$ not containing that variable, the quantifier may be removed.
\item[Step B:] Each conjunct with a quantifier $\#_i\CAPPSI_j$ is of the form $\#_i\pardisjunction{\disjunction{\CAPTHETA_1}{\CAPTHETA_2}}{\disjunction{\ldots}{\CAPTHETA_m}}$ where each $\CAPTHETA$ is an ion (or a quantifier subsentence governing a disjunction, as will be made clear shortly). An equivalent disjunction is obtained by reordering so that the disjuncts with a variable matching that of the quantifier are leftmost. By the Q shuffle exchange rule, the $\#_i$ can be moved in to govern just those disjuncts, resulting in $\#_i\CAPPSI_j$ becoming: $\disjunction{\#_i\pardisjunction{\CAPTHETA_1}{\CAPTHETA_2}}{\disjunction{\ldots}{\CAPTHETA_m}}$. With this transformation $\#_i$ binds all variables within its scope, and no other variables are in its scope.
\end{description}

When all quantifiers are pushed in in this manner, the result is a \GQL{}1 sentence whose quantifiers are all independent.
\end{PROOF}
...
For the reasons described previously, the method is guaranteed to halt on $\CAPPHI^*$.
Therefore there is a decision procedure for $\CAPPHI^*$, and hence $\CAPPHI$.
\end{PROOF}
