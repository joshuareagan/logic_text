...
Each step justified by the rule \Rule{Assume} initiates such a vertical line, and each step justified by \Rule{$\HORSESHOE$-Intro} closes a box extending to the previous \Rule{Assume} step, marking the downward boundary of its line.
For each step either the \Rule{Assume} rule is applied with no line number cited, or another rule is given with the numbers of the previous rows to which the rule was applied to derive that step. 

Consider the previous derivation of $\conjunction{\Cl}{\Bl}$ from $\conjunction{\Bl}{\Cl}$:
...
We begin with some basic top-down and bottom-up strategies for each connective, adding more later in section \ref{Sec:Shortcut Rule Strategies} when we add shortcut rules.
