...
The three we use most commonly are \mention{$\CAPPHI$} (phi), \mention{$\CAPTHETA$} (theta), and \mention{$\CAPPSI$} (psi).
...
Set membership is expressed with the `$\in$' symbol.
Let the Greek letters $\Delta$ (delta) and $\Gamma$ (gamma) stand for arbitrary sets.
We assert that $7$ is an element of set $\Delta$ and $3$ is an element of $\Gamma$ as follows: $7 \in \Delta$, $3 \in \Gamma$. On this notation,
...
We use lowercase Greek letters $\alpha$ (alpha) and $\beta$ (beta) as metavariables for elements. For example, if $\alpha$ and $\beta$ are odd numbers and $\Delta$ is the set of even numbers, then $\alpha + \beta \in \Delta$.
...
\noindent{}As you can probably guess, the \mention{$\notin$} symbol means \emph{is not an element of}. The symbolic notation for \mention{$\Delta$ is a proper subset of $\Gamma$} is $\Delta \subset \Gamma$. No set is a proper subset of itself.
...
\noindent{}The symbol \mention{$\cup$} means \emph{union}. Some examples:
...
The symbol \mention{$\cap$} means \emph{intersection}.
