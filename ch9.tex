...
In previous chapters we assumed that there are only two truth values. We now set aside that assumption to explore many-valued logics. 
We consider the possibility that there are varying degrees of truth and falsity, such that there are sentences that are neither wholly true nor wholly false.
...
The number of values ranges from three to infinity. 
The interpretations of the further values vary widely from one author to another, as do the motivations for introducing the additional values. 
...
He sketched the idea of a many-valued logic in 1920 and published a systematic account in 1930.\footnote{Both are reprinted in \citealp{Borkowski1970}.}
...
However, it is one thing to decide that $\rfrac{1}{2}$ is your third truth value and another thing to give a philosophical explanation of it. 
...
\begin{center}
	...
	\end{tabular}		
\end{center}
For example, the conjunction of a true sentence and an indeterminate sentence is indeterminate. 
It is instead true if the indeterminacy is resolved in favor of truth and false if the indeterminacy is resolved in favor of falsity. 
...
If the conditional $\horseshoe{\Al}{\Bl}$ is defined as equivalent to as $\disjunction{\negation{\Al}}{\Bl}$, as is often done in two-valued logic, then it too is indeterminate when $\Al$ and $\Bl$ are.
As a consequence $\horseshoe{\Al}{\Al}$ is equivalent to the law of excluded middle, $\disjunction{\negation{\Al}}{\Al}$, and $\horseshoe{\Al}{\Al}$ is not a logical truth.
This outcome is generally thought undesirable.
...
In two-valued logic a sentence is logically true iff it is true in all models. 
When there are more than two truth values, we must indicate which subset of the values are the designated values, i.e., those that are truth-like. 
The definition is revised: $\Al$ is a \idf{logical truth} iff it has a designated value in all models. 
...
\L{}ukasiewicz' innovations have opened the possibilities for a variety of other systems and ideas. 
...
For example, four-valued \L{}ukasiewicz logic has the values 0, $\rfrac{1}{3}$, $\rfrac{2}{3}$, and 1.\footnote{
...
Conjunction takes the minimum value of the conjuncts, and disjunction the maximum value of the disjuncts. 
...
Excluded middle is not logically true in any of these systems, though in the even-valued systems excluded middle is always truer than the contradiction $\conjunction{\Al}{\negation{\Al}}$.\footnote{Systems with more than one designated value were mentioned by Post, and this variation on \L{}ukasiewicz systems was studied by Slupecki and others.}
...
\L{}ukasiewicz also studied the cases where the set of truth values consists of all rational numbers, or all the real numbers, in the interval $[0,1]$. 
...
The set of logical truths in the three-valued logic is a subset of those in our traditional logic; if a sentence can be shown to be false in a two-valued model, then that model also falsifies the sentence in the three-valued system. 
...
\end{center}	
\noindent{}are logical truths in an n-valued system but not in a system with at least $n+1$ values. 
...
If we assign both $\Al$ and $\Bl$ the same intermediate value, then $\negation{\Bl}$ has an intermediate value and so $\conjunction{\Al}{\negation{\Bl}}$ has an intermediate value. 
But since $\Al$ and $\Bl$ have the same value, $\horseshoe{\Al}{\Bl}$ has the value 1 and $\negation{\parhorseshoe{\Al}{\Bl}}$ has the value 0. 
So, the whole conditional has an intermediate value, not 1.
\end{majorILnc}
...
\noindent{}\emph{Solution.} We give an argument that this sentence is true for all finite-valued logics. 
...
But since $\Al > \Bl$, $1-\Al < 1-\Bl$, and so the value of the left conditional is also the value of $\horseshoe{\Al}{\Cl}$ because that is the minimum value on that side.
\end{majorILnc}
...
We can think of sentential logic as the logic of truth functional operators, and quantificational logic as the logic of quantifiers and predicates. 
But there is a lot left out by this: possibility and necessity, deontic concepts like permissibility and obligation, doxastic concepts like belief and knowledge, and temporal concepts like past and future, just to name a few. 
Logics which attempt to capture these concepts are called modal logics. 
Sometimes the term \mention{modal logic} refers just to the logic of possibility and necessity, and at other times it refers to the broader class of modal logics just mentioned.
In this section we give a brief introduction to the logic of possibility and necessity. 
...
Intuitively, $\BOX\CAPPHI$ means that $\CAPPHI$ is necessarily true, while $\DIAMOND\CAPPHI$ means that $\CAPPHI$ is possibly true. 
\begin{majorILnc}{\LnpDC{Recursive definition of Sentences of MGSL}} The \nidf{sentences} \underdf{of \MGSL{}}{sentence} are defined recursively:
\begin{description}
...
\end{cenumerate}
\end{majorILnc}
\noindent{}Elements of $\integer{W}$, ``possible worlds'', are denoted by $\world{w},\world{w}_0,\world{w}_1,\world{w}_2$ and so on. 
...
Intuitively, the set $\integer{V}(\CAPPHI)$ is the set of possible worlds in which $\CAPPHI$ is true.  And $\integer{W}-\integer{V}(\CAPPHI)$, those worlds not in $\integer{V}(\CAPPHI)$, are worlds in which $\CAPPHI$ is false.  We use the \mention{$-$} symbol to indicate the complement of a set.\footnote{For example, relative to some domain, $\{ 1, 2, 3, 4\}$, the complement of the set $\{1, 2\}$ is $\{3, 4\}$.  We indicate the latter with the following notation: $\{ 1, 2, 3, 4\}-\{1, 2\}$.}
...
\end{majorILnc}
\noindent{}Equivalently put, a sentence $\CAPPHI$ is a modal truth \Iff it's true everywhere on all models $\IntA$; i.e. \Iff for every world $\world{w}$ in every model $\IntA$, $\CAPPHI$ is true in $\world{w}$ in $\IntA$. 
...
(\SF{}, still of much interest today, is the system we focus on here.)
...
We do not bring these up at all, instead focusing on the basics of relational semantics.
...
\end{menumerate} 
allow for models where (say) Cicero is not Tully, or Clark Kent is not Superman.
...
But, if we extend \GQL{} to capture identity claims, then we are able to find such sentences. 
